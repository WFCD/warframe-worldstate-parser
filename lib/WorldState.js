...
class WorldState {
  ...
  constructor(json, deps = defaultDeps) {
    ...
    const cetusSynd = data.SyndicateMissions.filter(syndicate => syndicate.Tag === 'CetusSyndicate');
    const cetusBountyEnd = timeDate.parseDate(cetusSynd.length > 0 ? cetusSynd[0] : { $date: 0 });
    /**
     * The current earth cycle
     * @type {CetusCycle}
     */
