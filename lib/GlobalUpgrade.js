...
class GlobalUpgrade {
  ...
  constructor(data, { translator, timeDate }) {
     ...
     * @type {Date}
     */
    this.start = new Date(data.Activation.$date ? data.Activation.$date.$numberLong : 1000 * data.Activation.sec);

    /**
     * The time and date at which the global upgrade stops being active
     * @type {Date}
     */
    this.end = new Date(data.ExpiryDate.$date ? data.ExpiryDate.$date.$numberLong : 1000 * data.ExpiryDate.sec);
