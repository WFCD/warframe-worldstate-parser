...
class Fissure extends WorldstateObject {
  ...
  constructor(data, { translator, timeDate }) {
     ...
     * @type {Date}
     */
    this.activation = new Date(data.Activation.$date ? data.Activation.$date.$numberLong : 1000 * data.Activation.sec);

    /**
     * The date and time at which the fissure will disappear
     * @type {Date}
     */
    this.expiry = new Date(data.Expiry.$date ? data.Expiry.$date.$numberLong : 1000 * data.Expiry.sec);
  }
