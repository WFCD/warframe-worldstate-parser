...
class WorldEvent extends WorldstateObject {
  ...
  constructor(data, {
     ...
     * @type {?number}
     */
    this.smallInterval = Number.parseInt(data.GoalInterim);

    /**
     * The second intermediate score goal
     * @type {?number}
     */
    this.largeInterval = Number.parseInt(data.GoalInterim2);
    ...
    (data.InterimRewards || []).forEach((reward, index) => {
      ...
      this.interimSteps[index] = {
        goal: Number.parseInt(data.InterimGoals[index]),
    ...
    if (data.IsMultiProgress) {
      data.Types.forEach((type, index) => {
        this.progressSteps[index] = {
          ...
          progressAmt: Number.parseInt(data.MultiProgress[index]),
        };
      });
