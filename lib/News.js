...
class News extends WorldstateObject {
  ...
  constructor(data, { mdConfig, timeDate }) {
     ...
     * @type {Date}
     */
    this.date = new Date(data.Date.$date ? Number(data.Date.$date.$numberLong) : 1000 * data.Date.sec);
    ...
    this.startDate = data.EventStartDate
      ? new Date(data.EventStartDate.$date ? Number(data.EventStartDate.$date.$numberLong) : 1000 * data.EventStartDate.sec)
      : null;
    ...
    this.endDate = data.EventEndDate
      ? new Date(data.EventEndDate.$date ? Number(data.EventEndDate.$date.$numberLong) : 1000 * data.EventEndDate.sec)
      : null;
  }
