...
class CetusCycle extends WorldstateObject {
  ...
  constructor(bountiesEndDate, { mdConfig, timeDate }) {
    super({ _id: { $oid: `cetusCycle${this.getCurrentCetusCycle(bountiesEndDate, timeDate).expiry.getTime()}` } });
  ...
  getCurrentCetusCycle(endTime, timeDate) {
    const cetusEnd = endTime || this.bountiesEndDate;
    const timeFuncs = timeDate || this.timeDate;
    const now = Date.now();
    
    const millisLeft = timeFuncs.fromNow(cetusEnd);
    ...
    return {
      dayTime,
      timeLeft: timeFuncs.timeDeltaToString(millisLeft),
      endTime,
