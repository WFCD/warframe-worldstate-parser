...
const offsetInMin = 22.583333333;

function getCurrentCetusCycle() {
  const now = Date.now();
  const secondsIntoCycle = Math.floor((now / 1000) + (offsetInMin * 60)) % timeInCycle;
  let secondsRemainingInCycle = secondsIntoCycle > nightTime ?
    timeInCycle - secondsIntoCycle : nightTime - secondsIntoCycle;

  const dayTime = secondsIntoCycle > nightTime;
...
class CetusCycle extends WorldstateObject {
  ...
  constructor({ mdConfig }) {
    super({ _id: { $oid: `cetusCycle${getCurrentCetusCycle().expiry.getTime()}` } });
