...
class DarkSector extends WorldstateObject {
   ...
   */
  constructor(data, { mdConfig, translator, Mission, DarkSectorBattle, Reward }) {
    super(data);

    const deps = { translator, Mission, DarkSectorBattle, mdConfig, Reward };
    ...
    this.defenderDeployemntActivation =
      new Date(data.DefenderInfo.DeploymentActivationTime.$date ? Number(data.DefenderInfo.DeploymentActivationTime.$date.$numberLong) : 1000 * data.DefenderInfo.DeploymentActivationTime.sec);
     ...
     * @type {?Mission}
     */
    this.mission = data.DefenderInfo.MissionInfo ? new Mission(data.DefenderInfo.MissionInfo, deps) : null;
     ...
     */
    this.history = data.History.map(b => new DarkSectorBattle(b));
  }
}
