...
class DarkSectorBattle {
  ...
  constructor(data) {
     ...
     * @type {Date}
     */
    this.start = new Date(data.Start.$date ? data.Start.$date.$numberLong : 1000 * data.Start.sec);

    /**
     * The date and time at which the battle ended
     * @type {Date}
     */
    this.end = new Date(data.End.$date ? data.End.$date.$numberLong : 1000 * data.End.sec);
  }
}
